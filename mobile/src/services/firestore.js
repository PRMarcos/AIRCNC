...
async function ListBookings(userId) {
  let dados = [];
  try {
    const snapshot = await Db.collection("Booking").where("userId", "==", userId).get();
    snapshot.forEach(doc => {
      const obj = doc.data();
      dados.push({ _id: doc.id, ...obj });
    });
...
async function UPpdateFinished(bookingId) {
  try {
    await Db.collection("Booking").doc(bookingId).update({ finished: true })
  } catch (error) {
    throw new Error("Something went wrong: " + error.code + " - " + error.message);
  }
}
...
export { AddSpot, ListOwnerSpots, ListAllSpots, AddUser, ListSpots, AddBooking, ListBookings, UPpdateFinished };
